...
- (id) initWithRect: (NSRect)rect andTitle: (NSString *)title
{
  ...
  if ((self = [super initWithFrame:rect pixelFormat:pixFmt]) != nil)
	...
	return ((void *)0);
      ...
      glsl.loc_font_texture = glGetUniformLocation(glsl.font_program, "color");
...
- (void) mlx_gl_draw_font:(mlx_img_list_t *)img andCtx:(mlx_img_ctx_t *)imgctx andX:(int)x andY:(int)y andColor:(int)color glyphX:(int)gx glyphY:(int)gy
{
  ...
  color_tab[1] = ((color&0xFF000000)>>24)/255.0;
  color_tab[2] = ((color&0xFF0000)>>16)/255.0;
  color_tab[3] = ((color&0xFF00)>>8)/255.0;
  color_tab[0] = ((color&0xFF)>>0)/255.0;
  glUseProgram(glsl.font_program);

  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, imgctx->texture);
  glUniform1i(glsl.loc_font_texture, 0);
  glUniform4fv(glsl.loc_font_color, 4, color_tab);
